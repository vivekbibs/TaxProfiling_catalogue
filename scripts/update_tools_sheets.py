...
def update_or_not(repo_url, local_release): 
    ...
    if remote_version == local_release:
        ...
        print(f"⚠️ Une nouvelle version est disponible ! ({local_release} -> {remote_version})")
        print("--------------------------------------------------")
        return True
...
if __name__ == "__main__":
    ...
    tools["Release"] = tools["Release"].astype(str).apply(
        lambda x: x[1:] if x.startswith('v') else x
    )
    ...
    tools["To update"] = tools.apply(
        lambda row: update_or_not(row["repo"], row["Release"]), 
        axis=1
    )
